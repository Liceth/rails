...
class Rails::ServerTest < ActiveSupport::TestCase
  ...
  class MyServer < Rails::Server
    class Options
      def parse!(args)
        options = {}
        OptionParser.new do |opts|
          opts.on("-x") { |v| options[:Custom] = v }
        end.parse!(args)
  ...
  def test_override_default_options
    ...
    assert_equal 4000, my_server.options[:Port]
  end

  def test_override_opt_parser
    original_args = ARGV.dup
    args = %w(-x)
    ARGV.replace args

    my_server = MyServer.new
    assert my_server.options[:Custom]
  ensure
    ARGV.replace original_args
  end
  ...
  private
    def parse_arguments(args = [])
      Rails::Command::ServerCommand.new([], args).send(:server_options)
    end
end
